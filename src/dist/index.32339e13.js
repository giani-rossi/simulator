const IVA = 0.16;
let monto, plazo, totalPagos, tasaAnual, fechaInicio, fechaPago, tasaMensual, mensualidad, intereses, impuestos, capital, insoluto, primerInteres, primerImpuesto, primerCapital, primerInsoluto, primerFechaPago, acumIntereses, acumImpuestos, acumCapital;
...
var establecerDatos = function() {
    ...
    if (plazoMensual === true) this.plazo = plazo;
    else if (plazoAnual === true) this.plazo = plazo * 12;
    else alert('No seleccionaste ningún tipo de plazo');
    switch(periodo){
        case 'semanal':
            let fechaFin = new Date(fechaInicio);
            fechaFin.setMonth(fechaFin.getMonth() + parseInt(plazo));
            let tiempo = fechaFin.getTime() - fechaInicio.getTime();
            let dias = Math.floor(tiempo / 86400000);
...
function calcularTotalPrestamo() {
    let totalPrestamo = 0;
    for(let i = 0; i < totalPagos; i++)totalPrestamo += mensualidad;
...
function Intereses() {
    ...
    } else intereses = tasaMensual * insoluto;
    return intereses;
}
function Impuestos() {
    if (primerImpuesto === 0) {
...
function Capital() {
    ...
    } else capital = mensualidad - Intereses() - Impuestos();
    return capital;
}
...
function simularPrestamo() {
            ...
            switch(columnas[j]){
                ...
                case 'Fecha':
                    if (primerFechaPago === true) {
                        ...
                        if (periodo === 'semanal') fechaPago.setDate(fechaPago.getDate() + 7);
                        else if (periodo === 'quincenal') fechaPago.setDate(fechaPago.getDate() + 15);
                        else if (periodo === 'mensual') fechaPago.setMonth(fechaPago.getMonth() + 1);
                    }
    ...
    // footer de la tabla
    for(let j1 = 0; j1 < columnas.length; j1++){
        let celda = document.createElement('td');
        let texto;
        switch(columnas[j1]){
            case 'No.':
